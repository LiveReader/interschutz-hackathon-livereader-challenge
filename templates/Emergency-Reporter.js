...
function shapeBuilder(data, TemplateAPI) {
	...
	const { body, border, banner, pin, callbackIndicator } = addBaseShape();
	...
	function addBaseShape() {
		const frame = SVGShape(`
			<g transform="matrix(1,0,0,1,-897.945,-896.731)">
				...
				<g id="banner" transform="matrix(4.16667,0,0,4.16667,-0.000104167,-0.000833333)">
					<g transform="matrix(0.24,0,0,0.24,2.5e-05,0.0002)">
						<path d="M3574.74,1979.17C3592.73,2067.05 3602.16,2157.93 3602.16,2251C3602.16,2343.4 3592.86,2433.64 3575.15,2520.83L929.129,2520.83C911.413,2433.64 902.112,2343.4 902.112,2251C902.112,2157.93 911.549,2067.05 929.536,1979.17L3574.74,1979.17Z" style="stroke:rgb(204,204,204);stroke-width:8.33px;"/>
					</g>
				</g>
		...
		const banner = frame.select("#banner");
		...
		function setBannerColor() {
			const theme = data.forceSimulation.svgElement.classList.contains("dark") ? "dark" : "light";
			banner
				.classed("gly_gray_fill", true)
				.classed("lighten", theme === "light")
				.classed("darken", theme === "dark");
		}
		const themeObserver = new MutationObserver(() => {
			setBannerColor();
		});
		themeObserver.observe(data.forceSimulation.svgElement, { attributes: true });
	...
	function addFullName() {
		...
		const nameShape = TextCollection(
			...
			[
				...
				ShapeStyle("font-size", "180px", true),
				ShapeStyle("class", "gly_gray.lighten", !!placeholder),
			]
		);
	...
	function addLocation() {
		const locationShape = TextCollection(
			...
			[
				ShapeStyle("class", "gly_text.dark", true),
				ShapeStyle("font-size", "120px", true),
	...
	function pendingLocation(locationShape, iteration) {
		shape.selectAll(".location").remove();
	...
	function addLabel() {
		const labelShape = TagShape(
			...
			TagStyle(
				[40, 140],
				[ShapeStyle("class", "gly_text.white", true), ShapeStyle("font-size", "160px", true)],
				[],
				140
			)
		);
	...
	function setCategory(categoryShape) {
		...
		categoryShape
			...
			.classed("gly_teal_fill", category === "ambulance")
			.classed("gly_green_fill", category === "police-department")
			.classed("gly_red_fill", category === "fire-department")
			.classed("gly_orange_fill", category == "control-center")
			.classed("gly_purple_fill", category == "automatic-system")
			.classed("gly_blue_fill", category == "civilian");
		if (
			category != "ambulance" &&
			...
			categoryShape.classed("gly_gray_fill", true).classed("lighten", true);
		}
	}
}
