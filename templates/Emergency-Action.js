...
function shapeBuilder(data, TemplateAPI) {
	...
	function setCategory(categoryShape) {
		...
		categoryShape
			...
			.classed("gly_purple_fill", category === "emergency-rescue")
	...
	function addLabel() {
		...
		const labelShape = TextCollection(
			...
			[
				...
				ShapeStyle("font-size", "300px", true),
		...
		const tagCollection = TagCollection(
			...
			TagStyle(
				...
				[
					...
					ShapeStyle("class", "emergency-action-tag-fill", true),
				],
				110
			)
		);
	...
	function scheduledIcon() {
		...
		`);
		icon.attr("stroke", "none").classed("gly_red_fill", true);
		return icon;
	}
	function inProgressIcon() {
		...
		`);
		icon.attr("stroke", "none").classed("gly_blue_fill", true);
		return icon;
	}
	function completedIcon() {
		...
		`);
		icon.attr("stroke", "none").classed("gly_green_fill", true);
		return icon;
	}
}

function addStyles() {
	let style = document.querySelector("[data-style='emergency-action']");
	if (style) return;
	style = document.createElement("style");
	style.setAttribute("data-style", "emergency-action");
	style.type = "text/css";
	style.innerHTML = `
		svg .emergency-action-tag-fill {
			fill: #f4f4f4;
		}
		svg.dark .emergency-action-tag-fill {
			fill: #404040;
		}
	`;
